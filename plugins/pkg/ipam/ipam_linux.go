...
func ConfigureIface(ifName string, res *current.Result) error {
	...
	DebugLog.Println("ConfigureIface ", ifName)
	if len(res.Interfaces) == 0 {
		return fmt.Errorf("no interfaces to configure")
	}
	DebugLog.Println("netlink.LinkByName ", ifName)
	link, err := netlink.LinkByName(ifName)
	if err != nil {
		return fmt.Errorf("failed to lookup %q: %v", ifName, err)
	}
	DebugLog.Println("netlink.LinkSetUp ", link)
	...
	for _, ipc := range res.IPs {
		DebugLog.Println("ipc  ", ipc, ipc.Interface, ipc.Address.String(), ipc.Gateway.String())
		...
		DebugLog.Println("netlink.AddrAdd  ", link, addr)
		if err = netlink.AddrAdd(link, addr); err != nil {
			return fmt.Errorf("failed to add IP addr %v to %q: %v", ipc, ifName, err)
		}

		gwIsV4 := ipc.Gateway.To4() != nil
		DebugLog.Println("ipc.Gateway   ", ipc.Gateway)
		if gwIsV4 && v4gw == nil {
			v4gw = ipc.Gateway
	...
	for _, r := range res.Routes {
		...
		gw := r.GW
		DebugLog.Println("ipc.Gateway  gw ", gw)
		if gw == nil {
		...
		DebugLog.Println("ip.AddRoute(&r.Dst, gw, link) ", r.Dst, gw, link)
