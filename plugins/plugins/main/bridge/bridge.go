...
func ensureBridge(brName string, mtu int, promiscMode, vlanFiltering bool) (*netlink.Bridge, error) {
	...
	macvlanlog.DebugLog.Println("ensureBridge :", brName, mtu, promiscMode, vlanFiltering)
	br := &netlink.Bridge{
	...
	macvlanlog.DebugLog.Println("err := netlink.LinkAdd(br) ", br)
...
func setupBridge(n *NetConf) (*netlink.Bridge, *current.Interface, error) {
	...
	macvlanlog.DebugLog.Println("setupBridge   ", *n)
	vlanFiltering := false
	if n.Vlan != 0 {
		vlanFiltering = true
	}
	// create bridge if necessary
	macvlanlog.DebugLog.Println("ensureBridge   ", n.BrName, n.MTU, n.PromiscMode, vlanFiltering)
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	macvlanlog.InitLog(AddBridgeLog)
	...
	err, iddb, ipdb := db.GetIp(dbconn, args.ContainerID)
	IpFromDB = ipdb
	IdFromDB = iddb
	if err != nil {
	...
	macvlanlog.DebugLog.Println("loadNetConf ", newStdinData)
	n, cniVersion, err := loadNetConf([]byte(newStdinData))
	if err != nil {
		return err
	}
	...
	macvlanlog.DebugLog.Println("n.HairpinMode && n.PromiscMode  ", n.HairpinMode && n.PromiscMode)
